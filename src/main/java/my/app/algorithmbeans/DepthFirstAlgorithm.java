...
public class DepthFirstAlgorithm extends Algorithm {
    ...
    public int[][] resolvePuzzle(int[][] sudokuPuzzle) {
        ...
        Stack<PuzzleStateObject> stack = new Stack<>();

        // the first stack member, has always be present
        ...
        Stack<Integer> possibleValuesForFirstEmptyCell =
                findPossibleValuesForCell(clonedFirstSudokuPuzzle, emptyFirstCell[0], emptyFirstCell[1]);
        stack.push(new PuzzleStateObject(clonedFirstSudokuPuzzle, possibleValuesForFirstEmptyCell,
                emptyFirstCell[0], emptyFirstCell[1]));
        ...
        while (!puzzleSolved) {
            ...
            // todo use method which returns next coordinates with which program worked before
            int[] emptyCell = findFirstEmptyCellInPuzzleAndReturnCoordinates(clonedSudokuPuzzle);
            if (emptyCell != null) {
                Stack<Integer> possibleValuesForEmptyCell =
                        findPossibleValuesForCell(clonedSudokuPuzzle, emptyCell[0], emptyCell[1]);
                if (!possibleValuesForEmptyCell.empty()) {
                    stack.push(new PuzzleStateObject(clonedSudokuPuzzle, possibleValuesForEmptyCell,
