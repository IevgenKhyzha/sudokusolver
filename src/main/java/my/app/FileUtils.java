            ...
            while ((line = br.readLine()) != null) {
                ...
                for (int numberPlaceInLine = 2; numberPlaceInLine <= 22; numberPlaceInLine = numberPlaceInLine + 2) {
                    ...
                    if (!EMPTY_PLACE.equals(String.valueOf(line.charAt(numberPlaceInLine)))) {
                        sudokuArray[lineInArray][columnInArray] = Integer.parseInt(String.valueOf(line.charAt(numberPlaceInLine)));
                    } else {
...
//                }
                ...
                for (int j = 0; j < 9; j++) {
                    ...
                    bw.write("<td>" + makeColorCell(resolvedSudokuArray[i][j].getNumber(), resolvedSudokuArray[i][j].isPredefined()) + "</td>");
                }
                bw.write("<td>|</td>");
                bw.write("</tr>");
            }
    ...
    private String makeColorCell(int number, boolean isPredefined) {
        return isPredefined ? "<font color = \"RED\">" + number + "</font>" : "<font color = \"BLUE\">" + number + "</font>";
    }
}
